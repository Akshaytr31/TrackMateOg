...
function TaskListTable({ tableData }) {
    ...
    const getStatusBadgeColor=(status)=>{
        switch (status){
            case 'Complete':return 'bg-green-100 text-green-500 border border-green-200'
        ...
        <table className="min-w-full">
            <thead>
                <tr className="">
                    ...
                    <th className="py-3 px-4 text-gray-800 font-medium text-[13px] hidden md:table-cell">Createdn on</th>
                </tr>
            </thead>
            <tbody>
                {tableData.map((task)=>(
                    <tr key={task._id} className="border-t border-gray-200">
                        ...
                        <td className="py-4 text-gray-700 text-nowrap hidden md:table-cell">{task.createdAt?moment(task.createdAt).format('Do MMM YYYY'):'N/A'}</td>
                    </tr>
                ))}
            </tbody>
        </table>
